...
class PegawaiOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make('Jumlah Pegawai', Fisik::where('kondisi_fisik', 'Normal')
                ->where('periode_id', $periodeId) // Filter berdasarkan periode_id terakhir
                ->count())
                ->description('Status Fisik Normal')
                ->descriptionIcon('heroicon-m-users')
                ->color('success'),
            Stat::make('Jumlah Pegawai', Fisik::where('kondisi_fisik', 'Kurus')
                ->where('periode_id', $periodeId) // Filter berdasarkan periode_id terakhir
                ->count())
                ->description('Status Fisik Kurus')
                ->descriptionIcon('heroicon-m-users')
                ->color('info'),
            Stat::make('Jumlah Pegawai', Fisik::where('kondisi_fisik', 'OverWeight')
                ->where('periode_id', $periodeId) // Filter berdasarkan periode_id terakhir
                ->count())
                ->description('Status Fisik OverWeight')
                ->descriptionIcon('heroicon-m-users')
                ->color('warning'),
            Stat::make('Jumlah Pegawai', Fisik::where('kondisi_fisik', 'Obesitas 1')
                ->where('periode_id', $periodeId) // Filter berdasarkan periode_id terakhir
                ->count())
                ->description('Status Fisik Obesitas Level 1')
                ->descriptionIcon('heroicon-m-users')
                ->color('danger'),
            Stat::make('Jumlah Pegawai', Fisik::where('kondisi_fisik', 'Obesitas 2')
                ->where('periode_id', $periodeId) // Filter berdasarkan periode_id terakhir
                ->count())
